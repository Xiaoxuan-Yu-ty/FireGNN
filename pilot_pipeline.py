...
def main():
    ...
    parser.add_argument('--dataset', type=str, default="Composite-k16",
                       choices=['Composite-k16', 'Composite-k30','NormExpression','RawExpression'],
                       help='Dataset to use')
    ...
    parser.add_argument("--train_ml", action='store_true', help='Train machine learning models.')
    ...
    # Load graph exists
    if args.dataset == 'Composite-k16':
        graph_file = "AD/data/composite_patient_k16.pkl"
    elif args.dataset == 'Composite-k30':
        graph_file = "AD/data/composite_patient_k30.pkl"
    elif args.dataset == 'RawExpression':
        graph_file = "AD/data/raw_expression_k16.pkl"
    elif args.dataset == 'NormExpression':
        graph_file = "AD/data/normalized_expression_k30.pkl"
    else:  
        raise ValueError("Invalid dataset type, please choose from ['Composite-k16', 'Composite-k30','NormExpression','RawExpression'].")
    ...
    if args.train_baselines:
        for model in args.models:
            cmd = f"python train/train_baseline.py --model {model} --dataset {args.dataset} --graph_file {graph_file} --output_dir {args.output_dir} --epochs {args.epochs}"
    ...
    if args.train_fuzzy:
        for model in args.models:
            cmd = f"python train/train_fuzzy.py --model {model} --dataset {args.dataset} --graph_file {graph_file} --output_dir {args.output_dir} --epochs {args.epochs}"
